...
const Arena: React.FC<ArenaProps> = ({ sport }) => {
  ...
  const { socket, isConnected, emitBasketballScoreUpdate, emitFootballScoreUpdate, emitCricketScoreUpdate, joinMatch, leaveMatch } = useSocket();
  const { addMatch, updateMatch, setMatchStatus } = useMatches();
  ...
  const handleAddTeam = async () => {
    if (newTeam.name.trim() && newTeam.members.some(member => member.trim())) {
      try {
        ...
        // Save to backend
        const savedTeam = await teamAPI.create(teamData);
  ...
  const handleEndMatch = async () => {
    if (currentMatch) {
      try {
        ...
        await matchAPI.updateStatus(currentMatch.id, 'finished');
        ...
        // Update global match context
        updateMatch(currentMatch.id, { status: 'finished', endTime: new Date().toISOString() });
  ...
  const startMatch = () => {
    ...
    if (currentMatch) {
      const updatedMatch = { ...currentMatch, status: 'live' as const };
      setCurrentMatch(updatedMatch);
      
      // Update in allMatches array
      setAllMatches(prev => 
        prev.map(match => 
      ...
      // Update in quickMatches if it's a quick match
      ...
      // Update global match context
      setMatchStatus(currentMatch.id, 'live');
    }
  ...
  useEffect(() => {
    if (socket && isConnected) {
      // Listen for basketball score updates
      socket.on('basketball-score-updated', (data: any) => {
        console.log('Received basketball score update:', data);
        if (data.scoringData) {
          setBasketballScore1(data.scoringData.score1 || 0);
          setBasketballScore2(data.scoringData.score2 || 0);
          setBasketballQuarter(data.scoringData.quarter || 1);
          setBasketballTime(data.scoringData.timeRemaining || 0);
          setBasketballIsTimerRunning(data.scoringData.isTimerRunning || false);
        }
      });

      // Listen for football score updates
      socket.on('football-score-updated', (data: any) => {
        console.log('Received football score update:', data);
        if (data.scoringData) {
          setLiveScore1(data.scoringData.score1 || 0);
          setLiveScore2(data.scoringData.score2 || 0);
          setTimeRemaining(data.scoringData.timeRemaining || 0);
          setIsTimerRunning(data.scoringData.isTimerRunning || false);
          setMatchPhase(data.scoringData.phase || 'pre-match');
        }
      });

      // Listen for cricket score updates
      socket.on('cricket-score-updated', (data: any) => {
        console.log('Received cricket score update:', data);
        if (data.scoringData) {
          setCricketRuns(data.scoringData.runs || 0);
          setCricketWickets(data.scoringData.wickets || 0);
          setCricketOvers(data.scoringData.overs || 0);
          setCricketBalls(data.scoringData.balls || 0);
        }
      });
    ...
    if (!basketballGameCompleted) {
      ...
      if (currentMatch && isConnected) {
        emitBasketballScoreUpdate({
          ...
          action: 'timer',
          scoringData: {
            score1: basketballScore1,
            score2: basketballScore2,
            quarter: basketballQuarter,
            timeRemaining: basketballTime,
            isTimerRunning: true
          }
